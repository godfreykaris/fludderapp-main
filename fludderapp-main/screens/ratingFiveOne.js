...
export default function RatingFiveOne({ navigation }) {
    ...
    const fifthRating = navigation.getParam('currentRating');
    let fifthOneRating = 0
    fifthOneRating = navigation.getParam('fifthOneRating');
    const [currentRating, setCurrentRating] = useState(fifthOneRating > 0 ? fifthOneRating : 0);

    const pressHandler = (fifthOneRating) => {
        setCurrentRating(fifthOneRating);
    }
    const pressHandler1 = () => {
        navigation.navigate('RatingFive', { answer, reviewText, firstRating, secondRating, thirdRating, fourthRating, fifthRating });
    }
    const pressHandler2 = () => {
        if (currentRating > 0)
            navigation.navigate('SubmitReviewSuccess', { answer, reviewText, firstRating, secondRating, thirdRating, fourthRating, fifthRating, currentRating });
        else
            ...
            <ScrollView style={{ backgroundColor: '#fff' }}>
                <View style={styles.container}>
                    ...
                    <View style={{ margin: 5, padding: 10, flexDirection: 'row', flexWrap: 'wrap', textAlign: 'center', marginTop: 40, marginBottom: 10 }}>
                        <Text style={{ fontSize: 22 }}>Rate for <Text style={{ fontWeight: 'bold' }}>Clarity  </Text></Text>
                    </View>
                    <View>
                        <Text style={styles.fludderTipText}>Fludder tip: How clear and focused was the individual in their response?</Text>
                    </View>
